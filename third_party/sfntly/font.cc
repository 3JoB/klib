...
Table* Font::GetTable(int32_t tag) {
  if (!HasTable(tag)) return NULL;
  return tables_[tag];
}

const TableMap* Font::GetTableMap() { return &tables_; }
...
void Font::BuildTableHeadersForSerialization(
    std::vector<int32_t>* table_ordering, TableHeaderList* table_headers) {
  ...
  int32_t table_offset =
      Offset::kTableRecordBegin + num_tables() * Offset::kTableRecordSize;
...
void Font::SerializeHeader(FontOutputStream* fos,
  ...
  std::copy(table_headers->begin(), table_headers->end(),
...
void Font::GenerateTableOrdering(std::vector<int32_t>* default_table_ordering,
  ...
  for (Int2Bool::iterator table = tables_in_font.begin(),
       ...
       table != table_end; ++table) {
    if (table->second == false) table_ordering->push_back(table->first);
  }
}
...
CALLER_ATTACH Font::Builder* Font::Builder::GetOTFBuilder(
    FontFactory* factory, WritableFontData* wfd,
...
bool Font::Builder::ReadyToBuild() {
  ...
  for (TableBuilderMap::iterator table_builder = table_builders_.begin(),
       ...
       table_builder != table_builder_end; ++table_builder) {
    if (!table_builder->second->ReadyToBuild()) return false;
  }
  return true;
}
...
void Font::Builder::ClearTableBuilders() { table_builders_.clear(); }
...
Table::Builder* Font::Builder::GetTableBuilder(int32_t tag) {
  if (HasTableBuilder(tag)) return table_builders_[tag];
  return NULL;
}
...
void Font::Builder::InterRelateBuilders(TableBuilderMap* builder_map) {
  ...
  if (horizontal_metrics_builder != NULL) {
    if (max_profile_builder != NULL) {
      ...
      if (num_glyphs >= 0) horizontal_metrics_builder->SetNumGlyphs(num_glyphs);
    }
  ...
  if (loca_table_builder != NULL) {
    if (max_profile_builder != NULL) {
      int32_t num_glyphs = max_profile_builder->NumGlyphs();
      if (num_glyphs >= 0) loca_table_builder->SetNumGlyphs(num_glyphs);
    }
...
void Font::Builder::ReadHeader(FontInputStream* is,
  ...
  for (int32_t table_number = 0; table_number < num_tables_; ++table_number) {
    ...
    if (!IsValidHeaderRegion(is->Length(), offset, length)) continue;
...
void Font::Builder::ReadHeader(ReadableFontData* fd, int32_t offset,
  ...
  if (num_tables_ > fd->Size() / Offset::kTableRecordSize) return;

  int32_t table_offset = offset + Offset::kTableRecordBegin;
  for (int32_t table_number = 0; table_number < num_tables_;
    ...
    if (!IsValidHeaderRegion(fd->Size(), offset, length)) continue;
...
void Font::Builder::LoadTableData(HeaderOffsetSortedSet* headers,
  ...
  for (HeaderOffsetSortedSet::iterator it = headers->begin(),
    ...
    if (header->length() > kMaxTableSize) continue;
