...
MemoryInputStream::MemoryInputStream()
    : buffer_(NULL), position_(0), length_(0) {}

MemoryInputStream::~MemoryInputStream() { Close(); }

int32_t MemoryInputStream::Length() { return length_; }

int32_t MemoryInputStream::Available() { return length_ - position_; }
...
bool MemoryInputStream::MarkSupported() { return false; }
...
int32_t MemoryInputStream::Read(std::vector<uint8_t>* b, int32_t offset,
                                int32_t length) {
  assert(b);
  if (!buffer_) {
#if !defined(SFNTLY_NO_EXCEPTION)
...
void MemoryInputStream::Unread(std::vector<uint8_t>* b, int32_t offset,
                               int32_t length) {
  assert(b);
