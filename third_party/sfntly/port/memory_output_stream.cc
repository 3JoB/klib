...
void MemoryOutputStream::Write(std::vector<uint8_t>* buffer, int32_t offset,
                               int32_t length) {
  assert(buffer);
...
void MemoryOutputStream::Write(uint8_t* buffer, int32_t offset,
                               int32_t length) {
  assert(buffer);
  if (offset >= 0 && length > 0) {
...
void MemoryOutputStream::Write(uint8_t b) { store_.push_back(b); }
...
size_t MemoryOutputStream::Size() { return store_.size(); }
