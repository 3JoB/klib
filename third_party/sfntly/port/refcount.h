...
#define DEBUG_OUTPUT(a)                                                \
  fprintf(stderr, "%s%s:oc=%d,oid=%d,rc=%d\n", a, typeid(this).name(), \
          object_counter_, object_id_, ref_count_)
#else
#define DEBUG_OUTPUT(a)
#endif
...
#if defined(ENABLE_OBJECT_COUNTER)
template <typename TDerived>
size_t RefCounted<TDerived>::object_counter_ = 0;
template <typename TDerived>
...
class Ptr {
 public:
  ...
  // lint does not like this but this is how it works.
  Ptr(T* pT) : p_(NULL) { *this = pT; }

  Ptr(const Ptr<T>& p) : p_(NULL) { *this = p; }
  ...
  bool operator<(const Ptr<T>& p) const { return (p_ < p.p_); }
