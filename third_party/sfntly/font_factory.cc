...
bool FontFactory::FingerprintFont() { return fingerprint_; }
...
CALLER_ATTACH Font::Builder* FontFactory::LoadSingleOTFForBuilding(
    WritableFontData* wfd, int32_t offset_to_offset_table) {
  // UNIMPLEMENTED: SHA-1 hash checking via Java DigestStream
...
void FontFactory::LoadCollectionForBuilding(WritableFontData* wfd,
  ...
  if (num_fonts > wfd->Length() / 4) return;
  ...
  for (int32_t font_number = 0; font_number < num_fonts;
    ...
    if (offset < 0 || offset >= wfd->Length()) continue;
