...
int32_t LocaTable::NumLocas() { return num_glyphs_ + 1; }
...
LocaTable::LocaTable(Header* header, ReadableFontData* data,
                     int32_t format_version, int32_t num_glyphs)
    : Table(header, data),
...
int32_t LocaTable::LocaIterator::Next() { return container()->Loca(index_++); }
...
std::vector<int32_t>* LocaTable::Builder::LocaList() { return GetLocaList(); }
...
int32_t LocaTable::Builder::NumLocas() { return GetLocaList()->size(); }
...
void LocaTable::Builder::SubDataSet() { Initialize(InternalReadData()); }
...
bool LocaTable::Builder::SubReadyToSerialize() { return !loca_.empty(); }
...
void LocaTable::Builder::Initialize(ReadableFontData* data) {
  ...
  if (data) {
    ...
    LocaTablePtr table =
        new LocaTable(header(), data, format_version_, num_glyphs_);
    Ptr<LocaTable::LocaIterator> loca_iter = new LocaTable::LocaIterator(table);
