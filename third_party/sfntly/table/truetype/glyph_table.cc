...
const int32_t GlyphTable::CompositeGlyph::kFLAG_WE_HAVE_AN_X_AND_Y_SCALE = 1
                                                                           << 6;
...
const int32_t GlyphTable::CompositeGlyph::kFLAG_SCALED_COMPONENT_OFFSET = 1
                                                                          << 11;
const int32_t GlyphTable::CompositeGlyph::kFLAG_UNSCALED_COMPONENT_OFFSET =
    1 << 12;
...
CALLER_ATTACH GlyphTable::Builder* GlyphTable::Builder::CreateBuilder(
...
void GlyphTable::Builder::Initialize(ReadableFontData* data,
  ...
  if (data != NULL) {
    ...
    for (size_t i = 1; i < loca.size(); ++i) {
      ...
      builder.Attach(
          Glyph::Builder::GetBuilder(this, data, last_loca_value /*offset*/,
...
CALLER_ATTACH GlyphTable::Glyph* GlyphTable::Glyph::GetGlyph(
    GlyphTable* table, ReadableFontData* data, int32_t offset, int32_t length) {
...
int32_t GlyphTable::Glyph::GlyphType() { return glyph_type_; }

int32_t GlyphTable::Glyph::NumberOfContours() { return number_of_contours_; }

int32_t GlyphTable::Glyph::XMin() { return data_->ReadShort(Offset::kXMin); }

int32_t GlyphTable::Glyph::XMax() { return data_->ReadShort(Offset::kXMax); }

int32_t GlyphTable::Glyph::YMin() { return data_->ReadShort(Offset::kYMin); }

int32_t GlyphTable::Glyph::YMax() { return data_->ReadShort(Offset::kYMax); }
...
GlyphTable::Glyph::Builder::GetBuilder(GlyphTable::Builder* table_builder,
                                       ReadableFontData* data) {
...
bool GlyphTable::Glyph::Builder::SubReadyToSerialize() { return true; }
...
void GlyphTable::SimpleGlyph::Initialize() {
  ...
  y_coordinates_offset_ =
      x_coordinates_offset_ + x_byte_count_ * DataSize::kBYTE;
  ...
  int32_t non_padded_data_length =
      5 * DataSize::kSHORT + (NumberOfContours() * DataSize::kUSHORT) +
      DataSize::kUSHORT + (instruction_size_ * DataSize::kBYTE) +
      (flag_byte_count_ * DataSize::kBYTE) + (x_byte_count_ * DataSize::kBYTE) +
...
void GlyphTable::CompositeGlyph::Initialize() {
  ...
  while ((flags & kFLAG_MORE_COMPONENTS) == kFLAG_MORE_COMPONENTS) {
    ...
    if (flags == ReadableFontData::kInvalidUnsigned) break;
