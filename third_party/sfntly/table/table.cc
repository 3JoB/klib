...
int64_t Table::CalculatedChecksum() { return data_->Checksum(); }

void Table::SetFont(Font* font) { font_ = font; }
 ...
 * Table::Builder class
 ******************************************************************************/
Table::Builder::~Builder() { header_.Release(); }
...
void Table::Builder::NotifyPostTableBuild(FontDataTable* table) {
  if (model_changed() || data_changed()) {
    ...
    derived_table->header_ =
        new Header(header()->tag(), derived_table->DataLength());
  }
}
...
Table::Builder::Builder(Header* header) { header_ = header; }
