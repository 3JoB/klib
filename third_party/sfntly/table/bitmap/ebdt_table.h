...
class EbdtTable : public SubTableContainerTable, public RefCounted<EbdtTable> {
 public:
  struct Offset {
    enum {
      kVersion = 0,
  ...
  class Builder : public SubTableContainerTable::Builder,
   ...
   private:
    ...
    static void Initialize(ReadableFontData* data, BitmapLocaList* loca_list,
  ...
  virtual ~EbdtTable();
  int32_t Version();
  CALLER_ATTACH BitmapGlyph* Glyph(int32_t offset, int32_t length,
                                   int32_t format);
