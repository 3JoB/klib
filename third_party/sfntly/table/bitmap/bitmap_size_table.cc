...
IndexSubTable* BitmapSizeTable::CreateIndexSubTable(int32_t index) {
  return IndexSubTable::CreateIndexSubTable(master_read_data(),
                                            IndexSubTableArrayOffset(), index);
}
...
void BitmapSizeTable::Builder::SubDataSet() { Revert(); }
...
int32_t BitmapSizeTable::Builder::SubDataSizeToSerialize() {
  ...
  for (IndexSubTableBuilderList::iterator b = builders->begin(),
    ...
    int32_t padding =
        FontMath::PaddingRequired(abs(sub_table_size), DataSize::kULONG);
...
CALLER_ATTACH BitmapSizeTable::Builder* BitmapSizeTable::Builder::CreateBuilder(
    WritableFontData* data, ReadableFontData* master_data) {
...
CALLER_ATTACH BitmapSizeTable::Builder* BitmapSizeTable::Builder::CreateBuilder(
    ReadableFontData* data, ReadableFontData* master_data) {
  BitmapSizeTableBuilderPtr output =
      new BitmapSizeTable::Builder(data, master_data);
  return output.Detach();
}
...
BitmapSizeTable::Builder::CreateIndexSubTableBuilder(int32_t index) {
  return IndexSubTable::Builder::CreateBuilder(
      master_read_data(), IndexSubTableArrayOffset(), index);
}
