 ...
 * EblcTable class
 ******************************************************************************/
int32_t EblcTable::Version() { return data_->ReadFixed(Offset::kVersion); }
...
// static
void EblcTable::CreateBitmapSizeTable(ReadableFontData* data, int32_t num_sizes,
                                      BitmapSizeTableList* output) {
  assert(data);
  assert(output);
  for (int32_t i = 0; i < num_sizes; ++i) {
    ReadableFontDataPtr new_data;
    new_data.Attach(down_cast<ReadableFontData*>(data->Slice(
        Offset::kBitmapSizeTableArrayStart + i * Offset::kBitmapSizeTableLength,
...
int32_t EblcTable::Builder::SubSerialize(WritableFontData* new_data) {
  ...
  int32_t sub_table_block_start_offset =
      size_table_offset +
  ...
  for (BitmapSizeTableBuilderList::iterator
    ...
    (*size_builder)
        ->SetIndexSubTableArrayOffset(current_sub_table_block_start_offset);
    ...
    for (IndexSubTableBuilderList::iterator
      ...
      index_sub_table_array_offset +=
          new_data->WriteUShort(index_sub_table_array_offset,
      ...
      int32_t padding =
          FontMath::PaddingRequired(current_sub_table_size, DataSize::kULONG);
#if defined(SFNTLY_DEBUG_BITMAP)
    ...
    (*size_builder)
        ->SetIndexTableSize(index_sub_table_offset -
                            current_sub_table_block_start_offset);
    WritableFontDataPtr slice_size_table;
    slice_size_table.Attach(
        down_cast<WritableFontData*>(new_data->Slice(size_table_offset)));
...
int32_t EblcTable::Builder::SubDataSizeToSerialize() {
  ...
  for (BitmapSizeTableBuilderList::iterator b = size_table_builders_.begin(),
    ...
    fprintf(stderr, "sizeIndex = %d, sizeBuilderSize=0x%x (%d)\n", size_index++,
            size_builder_size, size_builder_size);
#endif
...
void EblcTable::Builder::SubDataSet() { Revert(); }
...
// static
CALLER_ATTACH EblcTable::Builder* EblcTable::Builder::CreateBuilder(
...
void EblcTable::Builder::Initialize(ReadableFontData* data,
  ...
  if (num_sizes > data->Size() / Offset::kBitmapSizeTableLength) return;
