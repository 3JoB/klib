...
int32_t IndexSubTableFormat5::NumGlyphs() { return NumGlyphs(data_, 0); }
...
int32_t IndexSubTableFormat5::GlyphStartOffset(int32_t glyph_id) {
  ...
  int32_t loca = ReadFontData()->SearchUShort(
      EblcTable::Offset::kIndexSubTable5_glyphArray, DataSize::kUSHORT,
      NumGlyphs(), glyph_id);
  if (loca == -1) {
    return loca;
  }
...
CALLER_ATTACH BigGlyphMetrics* IndexSubTableFormat5::BigMetrics() {
  ...
  data.Attach(down_cast<ReadableFontData*>(
      data_->Slice(EblcTable::Offset::kIndexSubTable5_bigGlyphMetrics,
...
IndexSubTableFormat5::IndexSubTableFormat5(ReadableFontData* data,
  ...
  image_size_ =
      data_->ReadULongAsInt(EblcTable::Offset::kIndexSubTable5_imageSize);
}
...
int32_t IndexSubTableFormat5::NumGlyphs(ReadableFontData* data,
  ...
  int32_t num_glyphs = data->ReadULongAsInt(
      table_offset + EblcTable::Offset::kIndexSubTable5_numGlyphs);
  return num_glyphs;
}
...
int32_t IndexSubTableFormat5::Builder::GlyphStartOffset(int32_t glyph_id) {
  ...
  std::vector<int32_t>::iterator it =
      std::find(glyph_array->begin(), glyph_array->end(), glyph_id);
...
IndexSubTableFormat5::Builder::CreateBuilder(ReadableFontData* data,
                                             ...
                                             int32_t first_glyph_index,
                                             int32_t last_glyph_index) {
  int32_t length = Builder::DataLength(data, index_sub_table_offset,
  ...
  IndexSubTableFormat5BuilderPtr output = new IndexSubTableFormat5::Builder(
      new_data, first_glyph_index, last_glyph_index);
  return output.Detach();
}
...
CALLER_ATTACH FontDataTable* IndexSubTableFormat5::Builder::SubBuildTable(
  ...
  IndexSubTableFormat5Ptr output =
      new IndexSubTableFormat5(data, first_glyph_index(), last_glyph_index());
  return output.Detach();
}

void IndexSubTableFormat5::Builder::SubDataSet() { Revert(); }
...
int32_t IndexSubTableFormat5::Builder::SubSerialize(
  ...
  if (!model_changed()) {
    ...
    target.Attach(down_cast<WritableFontData*>(
        new_data->Slice(EblcTable::Offset::kIndexSubTable5_imageSize)));
    ...
    for (std::vector<int32_t>::iterator b = glyph_array_.begin(),
                                        e = glyph_array_.end();
         b != e; b++) {
...
void IndexSubTableFormat5::Builder::SetImageSize(int32_t image_size) {
  InternalWriteData()->WriteULong(EblcTable::Offset::kIndexSubTable5_imageSize,
                                  image_size);
}
...
void IndexSubTableFormat5::Builder::SetGlyphArray(
...
void IndexSubTableFormat5::Builder::Initialize(ReadableFontData* data) {
  ...
  if (data) {
    ...
    for (int32_t i = 0; i < num_glyphs; ++i) {
      glyph_array_.push_back(
          data->ReadUShort(EblcTable::Offset::kIndexSubTable5_glyphArray +
                           i * DataSize::kUSHORT));
    }
  }
}
...
int32_t IndexSubTableFormat5::Builder::DataLength(
    ReadableFontData* data, int32_t index_sub_table_offset,
    int32_t first_glyph_index, int32_t last_glyph_index) {
  int32_t num_glyphs =
      IndexSubTableFormat5::NumGlyphs(data, index_sub_table_offset);
