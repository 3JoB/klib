...
CALLER_ATTACH FontDataTable* IndexSubTableFormat2::Builder::SubBuildTable(
  ...
  IndexSubTableFormat2Ptr output =
      new IndexSubTableFormat2(data, first_glyph_index(), last_glyph_index());
  return output.Detach();
}

void IndexSubTableFormat2::Builder::SubDataSet() { Revert(); }

int32_t IndexSubTableFormat2::Builder::SubDataSizeToSerialize() {
  return EblcTable::Offset::kIndexSubTable2Length;
}

bool IndexSubTableFormat2::Builder::SubReadyToSerialize() { return true; }
...
int32_t IndexSubTableFormat2::Builder::SubSerialize(
  ...
  if (metrics_ == NULL) {
    ...
    source.Attach(
        down_cast<ReadableFontData*>(InternalReadData()->Slice(size)));
