...
CALLER_ATTACH BitmapGlyphInfo* IndexSubTable::GlyphInfo(int32_t glyph_id) {
  ...
  BitmapGlyphInfoPtr output = new BitmapGlyphInfo(
      glyph_id, image_data_offset(), GlyphStartOffset(glyph_id),
      GlyphLength(glyph_id), image_format());
...
// static
CALLER_ATTACH IndexSubTable* IndexSubTable::CreateIndexSubTable(
    ReadableFontData* data, int32_t offset_to_index_sub_table_array,
    int32_t array_index) {
...
IndexSubTable::IndexSubTable(ReadableFontData* data, int32_t first_glyph_index,
...
// static
int32_t IndexSubTable::CheckGlyphRange(int32_t glyph_id, int32_t first_glyph_id,
...
CALLER_ATTACH BitmapGlyphInfo* IndexSubTable::Builder::GlyphInfo(
    int32_t glyph_id) {
  BitmapGlyphInfoPtr glyph_info = new BitmapGlyphInfo(
      glyph_id, image_data_offset(), GlyphStartOffset(glyph_id),
      GlyphLength(glyph_id), image_format());
...
// static
CALLER_ATTACH IndexSubTable::Builder* IndexSubTable::Builder::CreateBuilder(
...
CALLER_ATTACH IndexSubTable::Builder* IndexSubTable::Builder::CreateBuilder(
  ...
  int32_t index_sub_table_offset =
      offset_to_index_sub_table_array + additional_offset_to_index_subtable;
...
int32_t IndexSubTable::Builder::SubDataSizeToSerialize() { return 0; }

bool IndexSubTable::Builder::SubReadyToSerialize() { return false; }
...
IndexSubTable::Builder::Builder(int32_t index_format, int32_t image_format,
                                int32_t image_data_offset, int32_t data_size)
    : SubTable::Builder(data_size),
      first_glyph_index_(0),
      last_glyph_index_(0),
      index_format_(index_format),
...
int32_t IndexSubTable::Builder::CheckGlyphRange(int32_t glyph_id) {
  return IndexSubTable::CheckGlyphRange(glyph_id, first_glyph_index(),
                                        last_glyph_index());
}

int32_t IndexSubTable::Builder::SerializeIndexSubHeader(
    WritableFontData* data) {
  int32_t size = data->WriteUShort(
      EblcTable::Offset::kIndexSubHeader_indexFormat, index_format());
