...
int32_t IndexSubTableFormat4::NumGlyphs(ReadableFontData* data,
  ...
  int32_t num_glyphs = data->ReadULongAsInt(
      table_offset + EblcTable::Offset::kIndexSubTable4_numGlyphs);
  return num_glyphs;
}
...
CALLER_ATTACH FontDataTable* IndexSubTableFormat4::Builder::SubBuildTable(
  ...
  IndexSubTableFormat4Ptr output =
      new IndexSubTableFormat4(data, first_glyph_index(), last_glyph_index());
  return output.Detach();
}

void IndexSubTableFormat4::Builder::SubDataSet() { Revert(); }

int32_t IndexSubTableFormat4::Builder::SubDataSizeToSerialize() {
  if (offset_pair_array_.empty()) {
    return InternalReadData()->Length();
  }
  return static_cast<int32_t>(EblcTable::Offset::kIndexSubHeaderLength) +
         DataSize::kULONG +
         GetOffsetArray()->size() *
...
int32_t IndexSubTableFormat4::Builder::SubSerialize(
  ...
  if (!model_changed()) {
    ...
    target.Attach(down_cast<WritableFontData*>(
        new_data->Slice(EblcTable::Offset::kIndexSubTable4_glyphArray)));
    ...
    for (std::vector<CodeOffsetPairBuilder>::iterator
             b = GetOffsetArray()->begin(),
             e = GetOffsetArray()->end();
         b != e; b++) {
...
void IndexSubTableFormat4::Builder::Initialize(ReadableFontData* data) {
  ...
  if (data) {
    ...
    for (int32_t i = 0; i < num_pairs; ++i) {
      int32_t glyph_code = data->ReadUShort(
          offset + EblcTable::Offset::kIndexSubTable4_codeOffsetPair_glyphCode);
      int32_t glyph_offset = data->ReadUShort(
          offset + EblcTable::Offset::kIndexSubTable4_codeOffsetPair_offset);
...
int32_t IndexSubTableFormat4::Builder::DataLength(
  ...
  int32_t num_glyphs =
      IndexSubTableFormat4::NumGlyphs(data, index_sub_table_offset);
...
IndexSubTableFormat4::Builder::BitmapGlyphInfoIterator::Next() {
  ...
  output =
      new BitmapGlyphInfo(glyph_code, container()->image_data_offset(), offset,
                          next_offset - offset, container()->image_format());
