...
CALLER_ATTACH FontDataTable* IndexSubTableFormat1::Builder::SubBuildTable(
  ...
  IndexSubTableFormat1Ptr output =
      new IndexSubTableFormat1(data, first_glyph_index(), last_glyph_index());
  return output.Detach();
}

void IndexSubTableFormat1::Builder::SubDataSet() { Revert(); }
...
int32_t IndexSubTableFormat1::Builder::SubSerialize(
  ...
  if (!model_changed()) {
    ...
    target.Attach(down_cast<WritableFontData*>(
        new_data->Slice(EblcTable::Offset::kIndexSubTable1_offsetArray)));
...
void IndexSubTableFormat1::Builder::Initialize(ReadableFontData* data) {
  ...
  if (data) {
    ...
    for (int32_t i = 0; i < num_offsets; ++i) {
      offset_array_.push_back(
          data->ReadULongAsInt(EblcTable::Offset::kIndexSubTable1_offsetArray +
                               i * DataSize::kULONG));
    }
  }
}
...
IndexSubTableFormat1::Builder::BitmapGlyphInfoIterator::Next() {
  ...
  output = new BitmapGlyphInfo(glyph_id_, container()->image_data_offset(),
