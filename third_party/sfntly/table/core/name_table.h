...
class NameTable : public SubTableContainerTable, public RefCounted<NameTable> {
 public:
  ...
  class NameEntry : public RefCounted<NameEntry> {
   public:
    NameEntry();
    NameEntry(const NameEntryId& name_entry_id,
  ...
  class NameEntryBuilder : public RefCounted<NameEntryBuilder> {
   public:
    ...
    virtual void SetName(const std::vector<uint8_t>& name_bytes, int32_t offset,
                         int32_t length);
 ...
 private:
  ...
  // Note: Output will be stored in std::vector<uint8_t>* b.  Original data in b
  // will be erased and replaced with converted name bytes.
