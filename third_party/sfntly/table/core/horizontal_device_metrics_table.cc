...
int32_t HorizontalDeviceMetricsTable::Width(int32_t record_index,
                                            int32_t glyph_num) {
  if (record_index < 0 || record_index >= NumRecords() || glyph_num < 0 ||
      glyph_num >= num_glyphs_) {
#if !defined(SFNTLY_NO_EXCEPTION)
    throw IndexOutOfBoundsException();
#endif
    return -1;
...
HorizontalDeviceMetricsTable::HorizontalDeviceMetricsTable(
    Header* header, ReadableFontData* data, int32_t num_glyphs)
...
HorizontalDeviceMetricsTable::Builder::SubBuildTable(ReadableFontData* data) {
  FontDataTablePtr table =
      new HorizontalDeviceMetricsTable(header(), data, num_glyphs_);
  return table.Detach();
}
