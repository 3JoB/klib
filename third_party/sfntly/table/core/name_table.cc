 ...
 ******************************************************************************/
NameTable::NameEntry::NameEntry() { Init(0, 0, 0, 0, NULL); }

NameTable::NameEntry::NameEntry(const NameEntryId& name_entry_id,
                                const std::vector<uint8_t>& name_bytes) {
  Init(name_entry_id.platform_id(), name_entry_id.encoding_id(),
       name_entry_id.language_id(), name_entry_id.name_id(), &name_bytes);
}
...
UChar* NameTable::NameEntry::Name() {
  return NameTable::ConvertFromNameBytes(&name_bytes_, platform_id(),
                                         encoding_id());
}
 ...
 ******************************************************************************/
NameTable::NameEntryBuilder::NameEntryBuilder() { Init(0, 0, 0, 0, NULL); }

NameTable::NameEntryBuilder::NameEntryBuilder(
    const NameEntryId& name_entry_id, const std::vector<uint8_t>& name_bytes) {
  Init(name_entry_id.platform_id(), name_entry_id.encoding_id(),
       name_entry_id.language_id(), name_entry_id.name_id(), &name_bytes);
}
...
NameTable::NameEntryBuilder::NameEntryBuilder(
  ...
  Init(name_entry_id.platform_id(), name_entry_id.encoding_id(),
       name_entry_id.language_id(), name_entry_id.name_id(), NULL);
}

NameTable::NameEntryBuilder::NameEntryBuilder(NameEntry* b) {
  Init(b->platform_id(), b->encoding_id(), b->language_id(), b->name_id(),
       b->NameAsBytes());
}
...
void NameTable::NameEntryBuilder::SetName(const UChar* name) {
  ...
  NameTable::ConvertToNameBytes(name, name_entry_->platform_id(),
                                name_entry_->encoding_id(),
                                &name_entry_->name_bytes_);
}

void NameTable::NameEntryBuilder::SetName(
    const std::vector<uint8_t>& name_bytes) {
  name_entry_->name_bytes_.clear();
...
void NameTable::NameEntryBuilder::SetName(
    const std::vector<uint8_t>& name_bytes, int32_t offset, int32_t length) {
  name_entry_->name_bytes_.clear();
...
NameTable::NameEntryIterator::NameEntryIterator(NameTable* table)
    : RefIterator<NameEntry, NameTable>(table), name_index_(0), filter_(NULL) {}
...
CALLER_ATTACH NameTable::Builder* NameTable::Builder::CreateBuilder(
...
int32_t NameTable::Builder::SubSerialize(WritableFontData* new_data) {
  ...
  for (NameEntryBuilderMap::iterator b = name_entry_map_.begin(),
    ...
    string_offset +=
        new_data->WriteBytes(string_offset + string_table_start_offset,
...
int32_t NameTable::Format() { return data_->ReadUShort(Offset::kFormat); }

int32_t NameTable::NameCount() { return data_->ReadUShort(Offset::kCount); }
...
UConverter* NameTable::GetCharset(int32_t platform_id, int32_t encoding_id) {
  ...
  UConverter* conv =
      ucnv_open(GetEncodingName(platform_id, encoding_id), &error_code);
...
void NameTable::ConvertToNameBytes(const UChar* name, int32_t platform_id,
  ...
  ucnv_fromUChars(cs, reinterpret_cast<char*>(&((*b)[0])), length + 4, name, -1,
                  &error_code);
  if (!U_SUCCESS(error_code)) {
    b->clear();
  }
