...
int32_t OS2Table::TableVersion() { return data_->ReadUShort(Offset::kVersion); }
...
int32_t OS2Table::FsType() { return data_->ReadUShort(Offset::kFsType); }
...
int32_t OS2Table::SxHeight() { return data_->ReadShort(Offset::kSxHeight); }

int32_t OS2Table::SCapHeight() { return data_->ReadShort(Offset::kSCapHeight); }
...
CALLER_ATTACH OS2Table::Builder* OS2Table::Builder::CreateBuilder(
...
void OS2Table::Builder::Panose(std::vector<uint8_t>* value) {
  ...
  InternalReadData()->ReadBytes(Offset::kPanose, &((*value)[0]), 0,
...
void OS2Table::Builder::SetAchVendId(std::vector<uint8_t>* b) {
  assert(b);
  assert(b->size());
  InternalWriteData()->WriteBytesPad(
      Offset::kAchVendId, b, 0,
      std::min<size_t>((size_t)Offset::kAchVendIdLength, b->size()),
      static_cast<uint8_t>(' '));
}
