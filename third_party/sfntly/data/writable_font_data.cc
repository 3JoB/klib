...
//                  savvy to do copying.
CALLER_ATTACH
WritableFontData* WritableFontData::CreateWritableFontData(
...
int32_t WritableFontData::WriteBytes(int32_t index, uint8_t* b, int32_t offset,
                                     int32_t length) {
  return array_->Put(BoundOffset(index), b, offset, BoundLength(index, length));
}
...
int32_t WritableFontData::WriteBytesPad(int32_t index, std::vector<uint8_t>* b,
                                        int32_t offset, int32_t length,
                                        uint8_t pad) {
  int32_t written = array_->Put(
      BoundOffset(index), &((*b)[0]), offset,
      BoundLength(index, std::min<int32_t>(length, b->size() - offset)));
...
void WritableFontData::CopyFrom(InputStream* is) { array_->CopyFrom(is); }
