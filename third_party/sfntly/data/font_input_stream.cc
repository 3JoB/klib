...
int32_t FontInputStream::Length() {
  if (bounded_) return length_;
  if (stream_) return stream_->Length();
  return 0;
}

int32_t FontInputStream::Available() {
  if (stream_) return stream_->Available();
  return 0;
}
...
int32_t FontInputStream::Read(std::vector<uint8_t>* b, int32_t offset,
                              int32_t length) {
...
int32_t FontInputStream::ReadChar() { return Read(); }
...
int64_t FontInputStream::ReadULong() { return 0xffffffffL & ReadLong(); }
...
int32_t FontInputStream::ReadFixed() { return ReadLong(); }
