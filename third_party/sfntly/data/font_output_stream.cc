...
void FontOutputStream::Write(std::vector<uint8_t>* b, int32_t off,
                             int32_t len) {
  assert(b);
  assert(stream_);
...
void FontOutputStream::WriteChar(uint8_t c) { Write(c); }
...
void FontOutputStream::WriteShort(int32_t s) { WriteUShort(s); }
...
void FontOutputStream::WriteLong(int64_t l) { WriteULong(l); }

void FontOutputStream::WriteFixed(int32_t f) { WriteULong(f); }
