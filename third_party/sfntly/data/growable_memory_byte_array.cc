...
int32_t GrowableMemoryByteArray::CopyTo(OutputStream* os, int32_t offset,
                                        int32_t length) {
  assert(os);
...
int32_t GrowableMemoryByteArray::InternalPut(int32_t index, uint8_t* b,
...
int32_t GrowableMemoryByteArray::InternalGet(int32_t index, uint8_t* b,
                                             int32_t offset, int32_t length) {
  memcpy(b + offset, &(b_[0]) + index, length);
  return length;
}

void GrowableMemoryByteArray::Close() { b_.clear(); }

uint8_t* GrowableMemoryByteArray::Begin() { return &(b_[0]); }
