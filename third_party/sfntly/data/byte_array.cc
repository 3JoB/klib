...
int32_t ByteArray::Get(int32_t index, uint8_t* b, int32_t offset,
                       int32_t length) {
  assert(b);
  if (index < 0 || index >= filled_length_) return 0;
...
int32_t ByteArray::Put(int32_t index, uint8_t* b, int32_t offset,
                       int32_t length) {
  assert(b);
  if (index < 0 || index >= Size()) {
#if defined(SFNTLY_NO_EXCEPTION)
...
int32_t ByteArray::CopyTo(int32_t dst_offset, ByteArray* array,
  ...
  while ((bytes_read = Get(index + src_offset, &(b[0]), 0, buffer_length)) >
         0) {
