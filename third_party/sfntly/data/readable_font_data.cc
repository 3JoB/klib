...
//                  savvy to do copying.
CALLER_ATTACH
ReadableFontData* ReadableFontData::CreateReadableFontData(
...
int32_t ReadableFontData::ReadBytes(int32_t index, uint8_t* b, int32_t offset,
                                    int32_t length) {
...
int32_t ReadableFontData::ReadUShort(int32_t index) {
  ...
  if (b1 < 0) return kInvalidUnsigned;
  int32_t b2 = ReadUByte(index + 1);
  if (b2 < 0) return kInvalidUnsigned;
  return 0xffff & (b1 << 8 | b2);
}

int32_t ReadableFontData::ReadShort(int32_t index) {
  int32_t b1 = ReadByte(index);
  if (b1 == kInvalidByte) return kInvalidShort;
  int32_t b2 = ReadUByte(index + 1);
  if (b2 < 0) return kInvalidShort;
...
int32_t ReadableFontData::ReadUInt24(int32_t index) {
  ...
  if (b3 < 0) return kInvalidUnsigned;
...
int64_t ReadableFontData::ReadULong(int32_t index) {
  ...
  if (b4 < 0) return kInvalidUnsigned;
...
int32_t ReadableFontData::ReadLong(int32_t index) {
  int32_t b1 = ReadByte(index);
  if (b1 == kInvalidByte) return kInvalidLong;
  int32_t b2 = ReadUByte(index + 1);
  if (b2 < 0) return kInvalidLong;
  int32_t b3 = ReadUByte(index + 2);
  if (b3 < 0) return kInvalidLong;
  int32_t b4 = ReadUByte(index + 3);
  if (b4 < 0) return kInvalidLong;
...
int64_t ReadableFontData::ReadDateTimeAsLong(int32_t index) {
  ...
  if (high == kInvalidUnsigned) return kInvalidLongDateTime;
  int64_t low = ReadULong(index + 4);
  if (low == kInvalidUnsigned) return kInvalidLongDateTime;
  return high << 32 | low;
}

int32_t ReadableFontData::ReadFWord(int32_t index) { return ReadShort(index); }
...
int32_t ReadableFontData::SearchUShort(int32_t start_index,
                                       int32_t start_offset, int32_t end_index,
                                       int32_t end_offset, int32_t length,
                                       int32_t key) {
  int32_t location = 0;
  int32_t bottom = 0;
  int32_t top = length;
...
#endif
      if (key <= location_end) return location;
...
int32_t ReadableFontData::SearchUShort(int32_t start_index,
                                       int32_t start_offset, int32_t length,
                                       int32_t key) {
  int32_t location = 0;
  int32_t bottom = 0;
  int32_t top = length;
  while (top != bottom) {
    location = (top + bottom) / 2;
    int32_t location_start = ReadUShort(start_index + location * start_offset);
    if (key == location_start) return location;
...
int32_t ReadableFontData::SearchULong(int32_t start_index, int32_t start_offset,
  ...
  while (top != bottom) {
    ...
    int32_t location_start =
        ReadULongAsInt(start_index + location * start_offset);
    if (key < location_start) {
      // location is below current location
      top = location;
    } else {
