...
void LoadFontBuilders(const char* font_path, FontFactory* factory,
                      FontBuilderArray* builders) {
  FileInputStream input_stream;
  input_stream.Open(font_path);
...
bool SerializeFont(const char* font_path, FontFactory* factory, Font* font) {
  if (!font_path || !factory || !font) return false;
...
#endif
  if (output_file == reinterpret_cast<FILE*>(NULL)) return false;
