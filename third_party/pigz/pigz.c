...
local void insert_track(struct mem_track_s *mem, void *ptr) {
  ...
  if (mem->num > mem->tot) mem->tot = mem->num;
  if (mem->size > mem->max) mem->max = mem->size;
  mem_track_drop(mem);
}
...
local void *malloc_track(struct mem_track_s *mem, size_t size) {
  ...
  if (ptr != NULL) insert_track(mem, ptr);
  return ptr;
}

local void *realloc_track(struct mem_track_s *mem, void *ptr, size_t size) {
  if (ptr == NULL) return malloc_track(mem, size);
...
#ifndef NOTHREAD
local void *yarn_malloc(size_t size) { return malloc_track(&mem_track, size); }

local void yarn_free(void *ptr) { free_track(&mem_track, ptr); }
#endif
...
local void *alloc(void *ptr, size_t size) {
  ...
  if (ptr == NULL) throw(ENOMEM, "not enough memory");
  return ptr;
}
...
local int log_show(void) {
  ...
  log_head = me->next;
  if (me->next == NULL) log_tail = &log_head;
#ifndef NOTHREAD
  twist(log_lock, BY, -1);
#endif
...
local void log_dump(void) {
  if (log_tail == NULL) return;
  while (log_show())
    ;
  log_free();
  if (mem_track.num || mem_track.size)
    complain("memory leak: %zu allocs of %zu bytes total", mem_track.num,
             mem_track.size);
  if (mem_track.max)
...
#define THREADABORT(ball)                                        \
  do {                                                           \
    if ((ball).code != EPIPE) complain("abort: %s", (ball).why); \
    drop(ball);                                                  \
    cut_short(-(ball).code);                                     \
  } while (0)
...
local inline size_t grow(size_t size) {
  ...
  size += size >> 2;
  top = size;
  for (shift = 0; top > 7; shift++) top >>= 1;
  if (top == 7) size = (size_t)1 << (shift + 3);
  if (size < 16) size = 16;
  if (size <= was) size = (size_t)0 - 1;
  return size;
}
...
local inline size_t vmemcpy(char **mem, size_t *size, size_t off, void *cpy,
                            size_t len) {
  size_t need;

  need = off + len;
  if (need < off) throw(ERANGE, "overflow");
  if (need > *size) {
    need = grow(need);
    if (off == 0) {
      RELEASE(*mem);
...
local size_t readn(int desc, unsigned char *buf, size_t len) {
  ...
  while (len) {
    ...
    if (ret < 0) throw(errno, "read error on %s (%s)", g.inf, strerror(errno));
    if (ret == 0) break;
    buf += ret;
    len -= (size_t)ret;
    got += (size_t)ret;
...
local length_t put_header(void) {
  ...
  if (g.form > 1) {  // zip
    ...
    len =
        put(g.outd, 4, (val_t)0x04034b50,  // local header signature
    ...
    len =
        put(g.outd, 1, (val_t)31, 1, (val_t)139, 1, (val_t)8,  // deflate
            1, (val_t)((g.name != NULL ? 8 : 0) + (g.comment != NULL ? 16 : 0)),
            4, (val_t)g.mtime, 1,
            (val_t)(g.level >= 9   ? 2
                    : g.level == 1 ? 4
                                   : 0),
            1, (val_t)3,  // unix
            0);
    if (g.name != NULL) len += writen(g.outd, g.name, strlen(g.name) + 1);
    if (g.comment != NULL)
...
local void put_trailer(length_t ulen, length_t clen, unsigned long check,
                       length_t head) {
  if (g.form > 1) {  // zip
    // write Zip64 data descriptor, as promised in the local header
    length_t desc = put(g.outd, 4, (val_t)0x08074b50, 4, (val_t)check, 8,
                        (val_t)clen, 8, (val_t)ulen, 0);
    ...
    if (g.comment != NULL) cent += writen(g.outd, g.comment, strlen(g.comment));
...
local unsigned long adler32z(unsigned long adler, unsigned char const *buf,
                             size_t len) {
...
local unsigned long crc32z(unsigned long crc, unsigned char const *buf,
                           size_t len) {
  while (len > UINT_MAX && buf != NULL) {
...
#define COMB(a, b, c) \
  (g.form == 1 ? adler32_comb(a, b, c) : crc32_comb(a, b, c))
...
local crc_t x2nmodp(size_t n, unsigned k) {
  ...
  while (n) {
    if (n & 1) p = multmodp(x2n_table[k & 31], p);
    n >>= 1;
    k++;
  }
  return p;
...
local struct space *get_space(struct pool *pool) {
  ...
  possess(pool->have);
  if (pool->limit == 0) wait_for(pool->have, NOT_TO_BE, 0);
...
local void grow_space(struct space *space) {
  ...
  if (more == space->size) throw(ERANGE, "overflow");
...
local void compress_thread(void *dummy) {
  ...
  try {
...
#endif
    {
      ...
      if (ret == Z_MEM_ERROR) throw(ENOMEM, "not enough memory");
      if (ret != Z_OK) throw(EINVAL, "internal error");
    }
    ...
    for (;;) {
      ...
      if (job->next == NULL) compress_tail = &compress_head;
      ...
      do {
        ...
        if (g.level <= 9) {
          ...
          outsize = 0;
          bits = 0;
          ZopfliDeflatePart(&g.zopts, 2, !(left || job->more), temp->buf,
                            temp->len, temp->len + len, &bits, &out, &outsize);
          ...
          if (left || job->more) {
            bits &= 7;
            if ((bits & 1) || !g.setdict) {
              if (bits == 0 || bits > 5) job->out->buf[job->out->len++] = 0;
...
local void append_len(struct job *job, size_t len) {
  ...
  if (job->lens == NULL) job->lens = get_space(&lens_pool);
  lens = job->lens;
  if (lens->size < lens->len + 3) grow_space(lens);
  if (len < 64)
...
local void parallel_compress(void) {
  ...
  do {
    ...
    if (++seq < 1) throw(ERANGE, "overflow");
...
local void single_compress(int reset) {
  ...
  do {
    ...
    if (g.level <= 9) {
      ...
      if (fresh) (void)deflateReset(strm);
      ...
      size = 0;
      bits = 0;
      ZopfliDeflatePart(&g.zopts, 2, !(more || left), in + hist, off - hist,
                        (off - hist) + got, &bits, &def, &size);
      bits &= 7;
      if (more || left) {
        if ((bits & 1) || !g.setdict) {
          writen(g.outd, def, size);
          if (bits == 0 || bits > 5) writen(g.outd, (unsigned char *)"\0", 1);
...
#define GETZC(str)                                                    \
  do {                                                                \
    ...
    have = 0;                                                         \
    do {                                                              \
      if (g.in_left == 0 && load() == 0) return -3;                   \
...
local int get_header(int save) {
  ...
  if (magic == 0x504b) {  // it's zip
    ...
    if (g.in_eof) return -3;
    if (magic == 0x0201 || magic == 0x0806)
      return -5;                     // central header or archive extra
    if (magic != 0x0403) return -4;  // not a local header
    g.zip64 = 0;
    SKIP(2);
    flags = GET2();
    if (flags & 0xf7f0) return -4;
    method = GET();  // return low byte of method or 256
    if (GET() != 0 || flags & 1) method = 256;  // unknown or encrypted
    if (save)
  ...
  // check header crc
  if ((flags & 2) && GET2() != (crc & 0xffff)) return -6;
...
local int more_zip_entries(void) {
  ...
  if (!g.list || g.verbosity < 2) return ret;
...
local void show_info(int method, unsigned long check, length_t len, int cont) {
  ...
  if (tag[max]) strcpy(tag + max - 3, "...");

  // convert time stamp to text
  if (g.stamp && !cont) {
    strcpy(mod, ctime(&g.stamp));
    now = time(NULL);
    if (strcmp(mod + 20, ctime(&now) + 20) != 0) strcpy(mod + 11, mod + 19);
  } else
    strcpy(mod + 4, "------ -----");
  mod[16] = 0;
  ...
  if (g.first) {
    if (g.verbosity > 1) fputs("method    check    timestamp    ", stdout);
    if (g.verbosity > 0) puts("compressed   original reduced  name");
    g.first = 0;
  }
  ...
  if (g.verbosity > 1 && g.hcomm != NULL) puts(g.hcomm);
}
  ...
  if (method < 0) {
    complain(method == -6   ? "skipping: %s corrupt: header crc error"
             : method == -1 ? "skipping: %s empty"
                            : "skipping: %s unrecognized format",
             g.inf);
    return;
  }
  ...
  if (g.form == 1) {
    ...
    if (at == -1) {
      ...
      do {
        ...
        while (len--) check = (check << 8) + *g.in_next++;
      } while (load() != 0);
      check &= LOW32;
    } else {
...
local void outb_write(void *dummy) {
  ...
  try {
    do {
      ...
      len = out_len;
      if (len && g.decode == 1) writen(g.outd, out_copy, len);
...
local int outb(void *desc, unsigned char *buf, unsigned len) {
  ...
  if (len) {
    if (g.decode == 1) writen(g.outd, buf, len);
...
local void infchk(void) {
  ...
  do {
    ...
    inflateBackEnd(&strm);
    if (ret == Z_DATA_ERROR)
      throw(EDOM, "%s: corrupted -- invalid deflate data (%s)", g.inf,
            strm.msg);
    if (ret == Z_BUF_ERROR)
      throw(EDOM, "%s: corrupted -- incomplete deflate data", g.inf);
    if (ret != Z_STREAM_END) throw(EINVAL, "internal error");
    ...
    if (g.form > 1) {     // zip local trailer (if any)
      if (g.form == 3) {  // data descriptor follows
        ...
        if (g.zip_crc ==
                SIG &&  // might be the signature
                        // if the expected CRC is not SIG, then it's a signature
            (g.out_check != SIG ||  // assume signature
                                    // now we're in a very rare case where CRC
                                    // == SIG -- the first four bytes could be
                                    // the signature or the CRC
             (g.zip_clen ==
                  SIG &&  // if not, then no signature
                          ...
                          // length that is *also* SIG (!) -- so check the low
                          // 32 bits of the expected compressed length for SIG
              ((clen & LOW32) !=
                   SIG ||  // assume signature and CRC
                           // now the expected CRC *and* the expected low 32
                           // bits of the compressed length are SIG -- this is
                           // so incredibly unlikely, clearly someone is messing
                           // with us, but we continue ... if the next four
                           // bytes are not SIG, then there is not a signature
                           // -- check those bytes, currently in g.zip_ulen:
               (g.zip_ulen ==
                    SIG &&  // if not, then no signature
                            // we have three SIGs in a row in the descriptor,
                            // and both the expected CRC and the expected clen
                            // are SIG
                            // -- the first one is a signature if we don't
                            // expect the third word to be SIG, which is either
                            // the low 32 bits of ulen, or if ZIP64, the high 32
                            // bits of clen:
      ...
      check += GET();
      if (g.in_eof) throw(EDOM, "%s: corrupted -- missing trailer", g.inf);
      if (check != g.out_check)
  ...
  if (cont > 1 && g.verbosity > 0) {
    if (g.verbosity > 1) printf("        %08lx                ", ktot);
    printf(
#if __STDC_VERSION__ - 0 >= 199901L || __GNUC__ - 0 >= 3
...
local void unlzw(void) {
  ...
  g.out_tot = 0;
  if (NOMORE()) throw(EDOM, "%s: lzw premature end", g.inf);
  flags = NEXT();
  if (flags & 0x60) throw(EDOM, "%s: unknown lzw flags set", g.inf);
  max = flags & 0x1f;
  if (max < 9 || max > 16) throw(EDOM, "%s: lzw bits out of range", g.inf);
  ...
  if (NOMORE())  // no compressed data is ok
    return;
  buf = NEXT();
  if (NOMORE()) throw(EDOM, "%s: lzw premature end", g.inf);  // need nine bits
  buf += NEXT() << 8;
  final = prev = buf & mask;  // code
  buf >>= bits;
  left = 16 - bits;
  if (prev > 255) throw(EDOM, "%s: invalid lzw code", g.inf);
  ...
  for (;;) {
    ...
    if (end >= mask && bits < max) {
      ...
      // machine instruction!)
      {
        ...
        if (rem) {
          rem = bits - rem;
          if (NOMORE()) break;  // end of compressed data
          while (rem > g.in_left) {
            rem -= g.in_left;
            if (load() == 0) throw(EDOM, "%s: lzw premature end", g.inf);
          }
          g.in_left -= rem;
          g.in_next += rem;
        }
    ...
    while (stack > OUTSIZE - outcnt) {
      while (outcnt < OUTSIZE) out_buf[outcnt++] = match[--stack];
      g.out_tot += outcnt;
      if (g.decode == 1) writen(g.outd, out_buf, outcnt);
      outcnt = 0;
    }
    do {
  ...
  g.out_tot += outcnt;
  if (outcnt && g.decode == 1) writen(g.outd, out_buf, outcnt);
}
...
local int copymeta(char *from, char *to) {
  ...
  if (stat(from, &st) != 0 || (st.st_mode & S_IFMT) != S_IFREG) return -4;
...
#endif
  if (ret == -1) throw(errno, "sync error on %s (%s)", g.outf, strerror(errno));
}
...
local void process(char *path) {
  ...
  static char *sufs[] = {".z",  "-z",  "_z",   ".Z",   ".gz",  "-gz",
                         ".zz", "-zz", ".zip", ".ZIP", ".tgz", NULL};
  ...
  if (path == NULL) {
    ...
    if (path != g.inf) vstrcpy(&g.inf, &g.inz, 0, path);
    len = strlen(g.inf);
    ...
    if ((st.st_mode & S_IFMT) != S_IFREG && (st.st_mode & S_IFMT) != S_IFIFO &&
        (st.st_mode & S_IFMT) != S_IFLNK && (st.st_mode & S_IFMT) != S_IFDIR) {
  ...
  if (path == NULL || g.pipeout) {
    ...
    if (!g.decode && !g.force && isatty(g.outd))
      throw(EINVAL,
            "trying to write compressed data to a terminal"
            " (use -f to force)");
  } else {
    char *to = g.inf, *sufx = "";
    size_t pre = 0;
    ...
    g.outd = open(g.outf, O_CREAT | O_TRUNC | O_WRONLY | (g.force ? 0 : O_EXCL),
                  0600);
  ...
  // process ind to outd
  if (g.verbosity > 1) fprintf(stderr, "%s to %s ", g.inf, g.outf);
  if (g.decode) {
    try {
      if (method == 8)
        infchk();
  ...
  load_end();
  if (g.outd != -1 && g.outd != 1) {
    if (g.sync) out_push();  // push to permanent storage
    if (close(g.outd))
      throw(errno, "write error on %s (%s)", g.outf, strerror(errno));
    ...
    if (g.decode && (g.headis & 2) != 0 && g.stamp) touch(g.outf, g.stamp);
  }
  RELEASE(g.outf);
}
...
#ifdef NOTHREAD
    "  specified, stdin will be compressed to stdout. pigz does what gzip "
    "does.",
#else
    "  specified, stdin will be compressed to stdout. pigz does what gzip "
    "does,",
    "  but spreads the work over multiple processors and cores when "
    "compressing.",
#endif
    "",
    "Options:",
...
local char *longopts[][2] = {
    {"LZW", "Z"},         {"lzw", "Z"},         {"alias", "A"},
    {"ascii", "a"},       {"best", "9"},        {"bits", "Z"},
    {"blocksize", "b"},   {"decompress", "d"},  {"fast", "1"},
    {"force", "f"},       {"comment", "C"},
#ifndef NOZOPFLI
    {"first", "F"},       {"iterations", "I"},  {"maxsplits", "J"},
    {"oneblock", "O"},
#endif
    {"help", "h"},        {"independent", "i"}, {"keep", "k"},
    {"license", "L"},     {"list", "l"},        {"name", "N"},
    {"no-name", "n"},     {"no-time", "m"},     {"processes", "p"},
    {"quiet", "q"},       {"recursive", "r"},   {"rsyncable", "R"},
    {"silent", "q"},      {"stdout", "c"},      {"suffix", "S"},
    {"synchronous", "Y"}, {"test", "t"},        {"time", "M"},
...
local size_t num(char *arg) {
  ...
  if (*str == 0) throw(EINVAL, "internal error: empty parameter");
  do {
    if (*str < '0' || *str > '9' ||
...
local int option(char *arg) {
  ...
  if (*arg == '-') {
    ...
    if (*arg == '-') {
      ...
      if (j < 0) throw(EINVAL, "invalid option: %s", arg - 2);
    }
    ...
    do {
      ...
      switch (*arg) {
        ...
        case 'V':
          puts(VERSION);
          if (g.verbosity > 1) printf("zlib %s\n", zlibVersion());
          exit(0);
          break;  // avoid warning
        case 'Y':
          g.sync = 1;
  ...
  if (get) {
    ...
    if (get == 1) {
      ...
      if (n != g.block >> 10 || OUTPOOL(g.block) < g.block ||
      ...
      if (g.procs < 1) throw(EINVAL, "invalid number of processes: %s", arg);
      if ((size_t)g.procs != n || INBUFS(g.procs) < 1)
        throw(EINVAL, "too many processes: %s", arg);
#ifdef NOTHREAD
      if (g.procs > 1) throw(EINVAL, "compiled without threads");
#endif
    } else if (get == 3) {
      if (*arg == 0) throw(EINVAL, "suffix cannot be empty");
...
local void cut_yarn(int err) { throw(err, "internal threads error"); }
#endif
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // check zlib version
    if (zlib_vernum() < 0x1230) throw(EINVAL, "zlib version less than 1.2.3");
    ...
    if (argc < 2 && isatty(g.decode ? 0 : 1)) help();
    ...
    for (n = 1; n < argc; n++)
      if (argv[n] != NULL) {
        if (done == 1 && g.pipeout && !g.decode && !g.list && g.form > 1)
          complain(
              "warning: output will be concatenated zip files"
