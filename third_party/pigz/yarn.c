...
local void fail(int err, char const *file, long line, char const *func) {
  ...
  if (yarn_abort != NULL) yarn_abort(err);
  exit(err);
}
...
local void *my_malloc(size_t size, char const *file, long line) {
  ...
  if ((block = my_malloc_f(size)) == NULL) fail(ENOMEM, file, line, "malloc");
  return block;
}
...
lock *new_lock_(long initial, char const *file, long line) {
  ...
  if (ret) fail(ret, file, line, "mutex_init");
  ret = pthread_cond_init(&(bolt->cond), NULL);
  if (ret) fail(ret, file, line, "cond_init");
  bolt->value = initial;
  return bolt;
}
...
void possess_(lock *bolt, char const *file, long line) {
  ...
  if (ret) fail(ret, file, line, "mutex_lock");
}

void release_(lock *bolt, char const *file, long line) {
  int ret = pthread_mutex_unlock(&(bolt->mutex));
  if (ret) fail(ret, file, line, "mutex_unlock");
}

void twist_(lock *bolt, enum twist_op op, long val, char const *file,
            long line) {
  if (op == TO)
    bolt->value = val;
  else if (op == BY)
  ...
  if (ret) fail(ret, file, line, "cond_broadcast");
...
void wait_for_(lock *bolt, enum wait_op op, long val, char const *file,
               long line) {
  switch (op) {
    case TO_BE:
      until(bolt->value == val) {
        int ret = pthread_cond_wait(&(bolt->cond), &(bolt->mutex));
        if (ret) fail(ret, file, line, "cond_wait");
      }
      break;
    case NOT_TO_BE:
      until(bolt->value != val) {
...
local void reenter(void *arg) {
  ...
  while ((match = *prior) != NULL) {
    if (pthread_equal(match->id, me)) break;
    prior = &(match->next);
  }
  if (match == NULL) fail(ESRCH, capsule->file, capsule->line, "reenter lost");
...
thread *launch_(void (*probe)(void *), void *payload, char const *file,
                long line) {
  ...
  if (ret) fail(ret, file, line, "attr_init");
  ret = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
  if (ret) fail(ret, file, line, "attr_setdetachstate");
  ret = pthread_create(&(th->id), &attr, ignition, capsule);
  if (ret) fail(ret, file, line, "create");
  ret = pthread_attr_destroy(&attr);
  if (ret) fail(ret, file, line, "attr_destroy");
...
void join_(thread *ally, char const *file, long line) {
  ...
  if (ret) fail(ret, file, line, "join");
  ...
  if (match == NULL) fail(ESRCH, file, line, "join lost");

  // remove thread from list and update exited count, free thread
  if (match->done) threads_lock.value--;
  *prior = match->next;
  release_(&(threads_lock), file, line);
  my_free(ally);
}
...
int join_all_(char const *file, long line) {
  ...
  while (threads != NULL) {
    ...
    if (match == NULL) fail(ESRCH, file, line, "join_all lost");
