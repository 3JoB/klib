...
ZSTDCB_DCtx *ZSTDCB_createDCtx(int threads, int inputsize) {
  ...
  /* check threads value */
  if (threads < 1 || threads > ZSTDCB_THREAD_MAX) return 0;
...
static int IsZstd_Magic(unsigned char *buf) {
  ...
  if (magic == ZSTDCB_MAGICNUMBER_V01) return 1;
  return (magic >= ZSTDCB_MAGICNUMBER_MIN && magic <= ZSTDCB_MAGICNUMBER_MAX);
}
...
static size_t pt_read(ZSTDCB_DCtx *ctx, ZSTDCB_Buffer *in, size_t *frame) {
  ...
  if (unlikely(ctx->frames == 0)) {
    ...
    if (unlikely(in->size != 16)) goto error_data;
    ctx->insize += 16;
    ...
    if (!IsZstd_Skippable(in->buf)) {
      ...
      if (in->size != toRead) goto error_data;
      ctx->insize += in->size;
      *frame = ctx->frames++;
      pthread_mutex_unlock(&ctx->read_mutex);
      return 0; /* done! */
    ...
    if (IsZstd_Skippable(in->buf)) {
      ...
      if (in->size != toRead - 4) goto error_data;
      ctx->insize += in->size;
  ...
  /* check header data */
  if (unlikely(hdr.size != 12)) goto error_read;
  if (unlikely(!IsZstd_Skippable(hdr.buf))) goto error_data;
  ctx->insize += 12;
...
static void *pt_decompress(void *arg) {
  ...
  for (;;) {
    ...
    if (!list_empty(&ctx->writelist_free)) {
      ...
      /* allocate new one */
      wl = (struct writelist *)malloc(sizeof(struct writelist));
      if (!wl) {
    ...
    again:
      ...
      if (ZSTD_isError(result)) goto error_clib;
      ...
      if (result == 0) {
        ...
        if (ZSTDCB_isError(result)) goto error_unlock;
        pthread_mutex_unlock(&ctx->write_mutex);
...
static size_t st_decompress(void *arg) {
  ...
  in->buf = malloc(in->size);
  if (!in->buf) return ZSTDCB_ERROR(memory_allocation);
  in->allocated = in->size;
  ...
  for (;;) {
    for (;;) {
      ...
      /* one more round */
      if ((zIn.pos == zIn.size) && (result == 1) && zOut.pos) continue;
...
size_t ZSTDCB_decompressDCtx(ZSTDCB_DCtx *ctx, ZSTDCB_RdWr_t *rdwr) {
  ...
  if (in->size < 16) {
    if (!IsZstd_Magic(buf)) return ZSTDCB_ERROR(data_error);
  ...
  if (type == TYPE_SINGLE_THREAD) {
    ...
    if (!w->dctx) return ZSTDCB_ERROR(memory_allocation);
