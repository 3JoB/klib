...
ZSTDCB_CCtx *ZSTDCB_createCCtx(int threads, int level, int inputsize) {
  ...
  /* check threads value */
  if (threads < 1 || threads > ZSTDCB_THREAD_MAX) goto err_ctx;

  /* check level */
  if (level < ZSTDCB_LEVEL_MIN || level > ZSTDCB_LEVEL_MAX) goto err_ctx;

  /* calculate chunksize for one thread */
  if (inputsize)
    ctx->inputsize = inputsize;
  else {
    const int windowLog[] = {19, 19, 20, 20, 20, /*  1 -  5 */
                             21, 21, 21, 21, 21, /*  6 - 10 */
...
static size_t pt_write(ZSTDCB_CCtx *ctx, struct writelist *wl) {
  ...
  if (wl->frame != ctx->curframe) return 0;
...
static void *pt_compress(void *arg) {
  ...
  if (!in.buf) return (void *)ZSTDCB_ERROR(memory_allocation);
  ...
  for (;;) {
    ...
    /* compress whole frame */
    {
      ...
      result = ZSTD_compress(outbuf + 12, out->size - 12, in.buf, in.size,
                             ctx->level);
    ...
    pthread_mutex_unlock(&ctx->write_mutex);
    if (ZSTDCB_isError(result)) goto error;
  }
