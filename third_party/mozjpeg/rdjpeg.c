...
#if !JPEG_RAW_READER
  return jpeg_read_scanlines(&source->dinfo, source->pub.buffer,
                             source->pub.buffer_height);
#else
  jpeg_read_raw_data(&source->dinfo, source->pub.plane_pointer,
...
start_input_jpeg(j_compress_ptr cinfo, cjpeg_source_ptr sinfo) {
  ...
  jpeg_mem_src(&source->dinfo, source->pub.input, source->pub.input_size);
  ...
  for (i = 0; i < cinfo->input_components; i++) {
    cinfo->comp_info[i].h_samp_factor =
        source->dinfo.comp_info[i].h_samp_factor;
    cinfo->comp_info[i].v_samp_factor =
        source->dinfo.comp_info[i].v_samp_factor;

    source->pub.plane_pointer[i] = (*cinfo->mem->alloc_sarray)(
        (j_common_ptr)cinfo, JPOOL_IMAGE, (JDIMENSION)cinfo->image_width,
        (JDIMENSION)NUM_ROWS);
  }
#endif
...
jinit_read_jpeg(j_compress_ptr cinfo) {
  ...
  source = (jpeg_source_ptr)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, sizeof(jpeg_source_struct));
