...
//--------------------------------------------------------------------------------------------------
/// \brief  Converts a sequence of UTF-8 code units to a sequence of UTF-32 code
/// points.
///
/// \details
///     This static member function reads an input sequence of UTF-8 code units
///     and converts it to an output sequence of UTF-32 code points.  It uses
///     the DFA to perform non-ascii code-unit sequence conversions, but
///     optimizes by converting contiguous sequences of ASCII code units using
///     SSE intrinsics.  It uses the `AdvanceWithBigTable` member function to
...
///     range.
/// \param pDst
///     A non-null pointer defining the beginning of the code point output
///     range.
///
/// \returns
///     If successful, the number of UTF-32 code points written; otherwise -1 is
///     returned to indicate an error was encountered.
//--------------------------------------------------------------------------------------------------
//
KEWB_ALIGN_FN std::ptrdiff_t UtfUtils::SseBigTableConvert(
    char8_t const* pSrc, char8_t const* pSrcEnd, char32_t* pDst) noexcept {
...
//--------------------------------------------------------------------------------------------------
/// \brief  Converts a sequence of ASCII UTF-8 code units to a sequence of
/// UTF-32 code points.
///
/// \details
///     This static member function uses SSE intrinsics to convert a register of
///     ASCII code units to four registers of equivalent UTF-32 code units.
///
/// \param pSrc
///     A reference to a non-null pointer defining the start of the code unit
///     input range.
/// \param pDst
///     A reference to a non-null pointer defining the start of the code point
///     output range.
//--------------------------------------------------------------------------------------------------
//
KEWB_FORCE_INLINE void UtfUtils::ConvertAsciiWithSse(char8_t const*& pSrc,
  ...
  chunk = _mm_loadu_si128(
      (__m128i const*)pSrc);        //- Load a register with 8-bit bytes
  ...
  qrtr =
      _mm_unpacklo_epi16(half, zero);  //- Unpack words 0-3 into 32-bit dwords
  _mm_storeu_si128((__m128i*)pDst, qrtr);  //- Write to memory
  qrtr =
      _mm_unpackhi_epi16(half, zero);  //- Unpack words 4-7 into 32-bit dwords
  _mm_storeu_si128((__m128i*)(pDst + 4), qrtr);  //- Write to memory

  half =
      _mm_unpackhi_epi8(chunk, zero);  //- Unpack bytes 8-15 into 16-bit words
  qrtr =
      _mm_unpacklo_epi16(half, zero);  //- Unpack words 8-11 into 32-bit dwords
  _mm_storeu_si128((__m128i*)(pDst + 8), qrtr);  //- Write to memory
  qrtr =
      _mm_unpackhi_epi16(half, zero);  //- Unpack words 12-15 into 32-bit dwords
  _mm_storeu_si128((__m128i*)(pDst + 12), qrtr);  //- Write to memory

  //- If no bits were set in the mask, then all 16 code units were ASCII, and
  // therefore
  ...
  //- Otherwise, the number of trailing (low-order) zero bits in the mask
...
//--------------------------------------------------------------------------------------------------
/// \brief  Returns the number of trailing 0-bits in an integer, starting with
/// the least
///         significant bit.
///
/// \details
///     This static member function uses compiler intrinsics to determine the
///     number of trailing (i.e., low-order) zero bits in an int32_t parameter.
///     For example, an input value of 8 (0000 1000) would return a value of 3;
///     an input value of 64 (0100 0000) would return a value of 6.
///
/// \param x
...
//
#if defined KEWB_PLATFORM_LINUX && \
    (defined KEWB_COMPILER_CLANG || defined KEWB_COMPILER_GCC)

KEWB_FORCE_INLINE int32_t UtfUtils::GetTrailingZeros(int32_t x) noexcept {
